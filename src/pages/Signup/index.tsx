...
export function Signup({ signInUser }: SignupProps) {

  const formik = useFormik({
    onSubmit: async (values) => {
      const res = await axios.post<User>(`${import.meta.env.VITE_API_HOST}/signup`, {
        name: values.name,
  ...
  return (
    <div className="h-full flex flex-col justify-center p-12 space-y-6">
      ...
      <span className="text-sm text-silver text-center">Já tem uma conta? <a className="text-birdBlue" href='/login'>Acesse.</a></span>
    </div>
  );
}